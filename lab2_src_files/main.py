...
if OBJECT == "pawn":
    ...
    T_ar_object = tfs.translation_matrix([-.06, .11, 0.091])
    # how many times to subdivide the mesh
    SUBDIVIDE_STEPS = 0
elif OBJECT == 'bar_clamp':
    MESH_FILENAME = 'objects/bar_clamp.obj'
    TAG = 9
    T_ar_object = tfs.translation_matrix([-.09, -.065, 0.035])
    SUBDIVIDE_STEPS = 1
elif OBJECT == "mount2":
    MESH_FILENAME = 'objects/mount2.obj'
    TAG = 10
...
def sorted_contacts(vertices, normals, T_ar_object):
    ...
    print(alpha)
    for i in range(min(10000, len(vertices))):
        i_index = np.random.choice(possible_indices, 2, replace=False)
        
        if np.linalg.norm(vertices[i_index[0]] - vertices[i_index[1]]) < MAX_HAND_DISTANCE and np.linalg.norm(vertices[i_index[0]] - vertices[i_index[1]]) > MIN_HAND_DISTANCE:
        	l = vertices[i_index[0]] - vertices[i_index[1]]
	        ...
	        grasp_indices.append(i_index)
	        best_metric_indices.append(i_index)
	        contacts = vertices[i_index]
	        contact_normals = normals[i_index]

        # YOUR CODE HERE
        	all_metrics.append(1)
...
if __name__ == '__main__':
    ...
    indeces, _ = sorted_contacts(vertices, normals, T_ar_object)
    # print(vertices[:5])
    # print(normals[:5])
    # print(indeces[:5])
    # print(type(indeces))
    ...
    # YOUR CODE HERE
    for current_metric in range(1):
    	normal1 = normals[indeces[0][0]]
    	normal2 = normals[indeces[0][1]]

    	contact1 = vertices[indeces[0][0]]
    	contact2 = vertices[indeces[0][1]]
        
    	print(normal1)
        print(normal1.reshape(-1, 1))
