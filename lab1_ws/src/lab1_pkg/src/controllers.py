...
class Controller:
    ...
    def execute_path(self, path, finished, timeout=None, log=False):
        ...
        while True:
            ...
            if finished is not None and finished(path, self.limb.endpoint_pose()):
                break
            r.sleep()
...
class PDWorkspaceVelocityController(Controller):
    ...
    def step_path(self, path, t):
        error = self.limb.endpoint_velocity()['linear'] - path.target_velocity(t)
        new_vel = self.Kp*error + self.Kv*(self.prev_error - error)
...
class PDJointVelocityController(Controller):
    ...
    def step_path(self, path, t):
        error = self.limb.joint_velocities() - path.target_velocity(t)
